...
<div class="main">
				 ...
				 <div class="inset">
				 	<div class="social-icons" style="background:#67adf3;">
		    			 <div class="span"><img src="images/fb.png" alt=""/><i>网易云音乐评价系统——第一轮调查 </i><div class="clear"></div></div>	
    						<div class="clear"></div>
					</div>
				 </div>	
				 ...
				 <form action="" method="post">
							<div class="lable-2">
		                    	...
		                    	<div style="width=-webkit-fill-available;">
				                    ...
				                    $id=1;
				                    $j=0;
				                    $query = $db1->query("SELECT q_num,question FROM Question");
									if($query) {
										while($row = $query->fetch_assoc()){							
											echo "<label>".$row["q_num"].".&nbsp;&nbsp;".$row["question"]."<br>";
											$j=$j+1;
											for($i=1;$i<6;$i++){
		                    ...
		                    <div class="lable">
		                       	...
		                       	<input style="position:relative;top:-40px;" type="text" class="text" name="suggest" value="其他意见或建议" onfocus="this.value = '';"}"></div>	   					
							<div class="clear"> </div>
								 <div class="submit">
...
//数据库连接参数 
	...
	$xhwdd = empty($_POST['2'])? die('<div class="main"><h2>请完成提问2！</h2></div>'): 
	 ...
	 $yhhd = empty($_POST['7'])? die('<div class="main"><h2>请完成提问7！</h2></div>'): 
	 mysqli_real_escape_string($db2,$_POST['7']); 
	 
	 $qcjsl = empty($_POST['8'])? die('<div class="main"><h2>请完成提问8！</h2></div>'): 
	 mysqli_real_escape_string($db2,$_POST['8']); 
	 
	 $fwwgjqd = empty($_POST['9'])? die('<div class="main"><h2>请完成提问9！</h2></div>'): 
	 mysqli_real_escape_string($db2,$_POST['9']); 
	 
	 $btiptx = empty($_POST['10'])? die('<div class="main"><h2>请完成提问10！</h2></div>'): 
	 mysqli_real_escape_string($db2,$_POST['10']); 
	 
	 $wlhz = empty($_POST['11'])? die('<div class="main"><h2>请完成提问11！</h2></div>'): 
	 mysqli_real_escape_string($db2,$_POST['11']); 
	 
	 $yjgxpl = empty($_POST['12'])? die('<div class="main"><h2>请完成提问12！</h2></div>'): 
	 mysqli_real_escape_string($db2,$_POST['12']); 
	 
	 $suggest = empty($_POST['suggest'])? die("请完成建议"): 
	 mysqli_real_escape_string($db2,$_POST['suggest']); 
	// echo $number." ".$wldk." ".$xhwdd." ".$fgqy." ".$wfjg." ".$lltx." ".$sylljz." ".$yhhd." ".$qcjsl." ".$fwwgjqd." ".$btiptx." ".$wlhz." ".$yjgxpl." ".$suggest;
	//打开数据库连接 hovertree.com 何问起 
	//构造一个SQL查询	
	$query = "INSERT INTO round1(number,CommentQuality,CommentUserQuality,CommentQualityWithSong,Recommend,SongNumFamous,SongNumMinority,ToneQuality,LyricQuality,RankingList,SongList,Privacy,Ad,suggest) VALUE('$number','$wldk','$xhwdd','$fgqy','$wfjg','$lltx','$sylljz','$yhhd','$qcjsl','$fwwgjqd','$btiptx','$wlhz','$yjgxpl','$suggest')"; 
	//执行该查询 
	$result = $db2->query($query) or die('<div class="main"><h2>该学号已接受过调查！</h2></div>'); 
	//插入操作成功后，显示插入记录的记录号 
	echo '<div class="main"><h2>提交成功！</h2></div><br><div class="submit"><a href="/test2.php"><h2>进入第二轮调查</h2></a></div> ' ; 
	//关闭当前数据库连接 
	$db2->close();
} 
?>
